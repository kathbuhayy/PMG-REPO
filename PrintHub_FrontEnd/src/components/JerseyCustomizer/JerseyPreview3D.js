// src/components/JerseyCustomizer/JerseyPreview3D.js
...
// ── Jersey/Sando-specific mesh + UV map ─────────────────────────────
// Real mesh names and UV bounds inside sando.glb, straight from its
// per-mesh UV report. Update THIS block (and nowhere else) whenever
// the model is re-exported.
const FRONT_MESH = "FRONT";
const BACK_MESH = "BACK";
const NECK_MESH = "NECK";
const LEFT_HEM_MESH = "LEFT_HEM";
const RIGHT_HEM_MESH = "RIGHT_HEM";
const BOTTOM_HEM_MESH = "BOTTOM_HEM";

const BODY_ZONES = {
  front: { meshName: FRONT_MESH, uMin: 0.01, uMax: 0.74, vMin: 0.01, vMax: 0.99 },
  back: { meshName: BACK_MESH, uMin: 0.01, uMax: 0.74, vMin: 0.01, vMax: 0.99 },
  neck: { meshName: NECK_MESH, uMin: 0.01, uMax: 0.77, vMin: 0.01, vMax: 0.99 },
  left_hem: { meshName: LEFT_HEM_MESH, uMin: 0.01, uMax: 0.78, vMin: 0.01, vMax: 0.99 },
  right_hem: { meshName: RIGHT_HEM_MESH, uMin: 0.01, uMax: 0.77, vMin: 0.01, vMax: 0.99 },
  bottom_hem: { meshName: BOTTOM_HEM_MESH, uMin: 0.01, uMax: 0.99, vMin: 0.97, vMax: 0.99 },
};

// Only front/back are ever decal/text-printable - neck/hems are
// color-only, matching tshirt's hem behavior.
const PRINTABLE_ZONE_IDS = ["front", "back"];

const meshNameToZone = {};
Object.entries(BODY_ZONES).forEach(([zoneId, cfg]) => {
  meshNameToZone[cfg.meshName] = zoneId;
});

export default function JerseyPreview3D({
  modelPath,
  shirtColor = "#ffffff",
  zoneColors = {}, // { neck, left_hem, right_hem, bottom_hem } -> hex overrides
  zoneDesigns = {},
  zoneTexts = {},
  zoneLayers = {},
  zones = [],
  ...
  useEffect(() => {
    ...
    const raycastBodyZone = (clientX, clientY) => {
      ...
      const targets = PRINTABLE_ZONE_IDS
        .map((zoneId) => model.getObjectByName(BODY_ZONES[zoneId].meshName))
        .filter(Boolean);
