...
// ── Jersey/Sando-specific mesh map ──────────────────────────────────
// Real mesh/node names inside sando.glb, one mesh per printable zone.
// Update THIS list (and nowhere else) whenever the model is re-exported.
const JERSEY_ZONE_MESH_MAP = {
  front: ["FRONT"],
  back: ["BACK"],
};

// Hem/trim meshes that support independent color overrides, separate
// from decal zones above. Matches the tshirt customizer's HEM_ZONE pattern.
const JERSEY_HEM_MESH_MAP = {
  neck: ["NECK"],
  left_hem: ["LEFT_HEM"],
  right_hem: ["RIGHT_HEM"],
...
export default function JerseyPreview3D({
  modelPath,
  shirtColor = "#ffffff",
  zoneColors = EMPTY_OBJECT, // { neck, left_hem, right_hem, bottom_hem } -> hex color overrides
  ...
  // Base color applies to every mesh EXCEPT hem meshes that have their
  // own explicit zoneColors override - matches tshirt customizer behavior.
  const applyBaseColor = useCallback(() => {
    const model = modelRef.current;
    if (!model) return;
    const color = new THREE.Color(colorRef.current);

    const hemMeshNameToZone = {};
    Object.entries(JERSEY_HEM_MESH_MAP).forEach(([zoneId, names]) => {
      names.forEach((n) => {
        hemMeshNameToZone[n] = zoneId;
      });
    });
    ...
    model.traverse((node) => {
      ...
      const hemZoneId = hemMeshNameToZone[node.name];
      const override = hemZoneId ? zoneColorsRef.current[hemZoneId] : null;
      if (node.material.color) {
        node.material.color = new THREE.Color(override || colorRef.current);
  ...
  const getZoneTargetMeshes = useCallback((model, zoneId) => {
    const names = JERSEY_ZONE_MESH_MAP[zoneId];
  ...
  const rebuildDecals = useCallback(() => {
    ...
    decalZoneIds.forEach((zoneId) => {
      ...
      const placeMesh = () => {
        ...
        const zoneW = zoneSize.x * (scale.w ?? 0.56);
        ...
        const decalSize = new THREE.Vector3(
          zoneW,
          zoneH,
          Math.max(zoneSize.z * (scale.depth ?? 0.22), 0.01),
        );
  ...
  useEffect(() => {
    ...
    applyBaseColor();
    rebuildDecalsThrottled();
  }, [applyBaseColor, rebuildDecalsThrottled, shirtColor, zoneColors]);
